...
# Inisialisasi database otomatis saat plugin dimuat
...
async def start_handler(client: Client, message: Message):
    ...
    if len(args) > 1:
        try:
            ...
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("💌 Kirim pesan teks", callback_data=f"send_type_text_{target_id}")],
                [InlineKeyboardButton("📸 Kirim foto rahasia", callback_data=f"send_type_photo_{target_id}")]
            ])

            await message.reply_text(
                "🔥 Kirimkan saya pesan anonim atau hadiah:\n\n"
                "Pilih salah satu metode di bawah buat mulai ngerusuh atau curhat ke dia secara misterius, bre! 👇✨",
    ...
    text = (
        f"Wih, halo bro! Selamat datang di markas rahasia lu 🤫🔥\n\n"
        f"🔗 Tautan Pribadi-mu:\n{my_link}\n\n"
        f"💡 Tambahkan tautan ini ke bio akunmu dan tunggu pesan anonim masuk! 💌✨"
    )
    
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("↗️ Bagikan", switch_inline_query="share")]
    ])
...
async def handle_anonymous_message(client: Client, message: Message):
    ...
    if send_type == "text":
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            await db.execute('INSERT INTO inbox (owner_id, sender_text, msg_type, created_at) VALUES (?, ?, ?, ?)', (target_id, content_text, 'text', now))
            await db.commit()
...
def setup(client: Client):
    ...
    client.on_message(filters.command("inbox"))(inbox_handler)
