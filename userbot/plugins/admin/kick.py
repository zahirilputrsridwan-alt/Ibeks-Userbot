...
def setup(client):
    ...
    async def cmd_kick(client, message):
        ...
        if not is_group(chat):
            await send_ui(client, chat.id, "Perintah ini hanya bisa digunakan di grup.", "KICK", "ADMIN", "ERROR", expandable=True)
            return
        ...
        if not target_id:
            await send_ui(client, chat.id, "Target tidak ditemukan. Reply ke pesan user atau berikan username/ID.", "KICK", "ADMIN", "ERROR", expandable=True)
            return

        if await is_self_target_async(client, target_id):
            await send_ui(client, chat.id, "Tidak bisa kick diri sendiri.", "KICK", "ADMIN", "ERROR", expandable=True)
            return
        ...
        try:
            ...
            await send_ui(client, chat.id, "User berhasil dikick dari grup.", "KICK", "ADMIN", "SUCCESS", expandable=True)
