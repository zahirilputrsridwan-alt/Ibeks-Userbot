...
def setup(client):
    ...
    async def cmd_promote(client, message):
        ...
        if not is_group(chat):
            await send_ui(client, chat.id, "Perintah ini hanya bisa digunakan di grup.", "PROMOTE", "ADMIN", "ERROR", expandable=True)
            return
        ...
        if not target_id:
            await send_ui(client, chat.id, "Target tidak ditemukan. Reply ke pesan user atau berikan username/ID.", "PROMOTE", "ADMIN", "ERROR", expandable=True)
            return

        if await is_self_target_async(client, target_id):
            await send_ui(client, chat.id, "Tidak bisa promote diri sendiri.", "PROMOTE", "ADMIN", "ERROR", expandable=True)
            return
        ...
        try:
            ...
            await send_ui(client, chat.id, "User berhasil dipromote menjadi admin.", "PROMOTE", "ADMIN", "SUCCESS", expandable=True)
